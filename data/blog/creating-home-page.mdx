...
![Image of overview of the home screen](/images/blog/usage-home-overview.png)
...
- You can set buttons in the `Hero section`. These buttons can be linked to blog posts or static pages within the website or external URLs. Set `enable` to `true` to enable the button. if enabled, `text` and `href` must be specified.
